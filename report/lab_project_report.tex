...
The synthetic diagnostics are useful because they check whether the network learns a sensible mapping between return windows and GARCH-style parameters. Figure \ref{fig:synth-diag} summarizes this behavior. The $\omega$ parameter is recovered with the clearest monotonic relationship, while $\alpha$ and $\beta$ are pushed toward a narrower central band. This is a reasonable outcome for pretraining: many combinations of $(\alpha,\beta)$ can generate similar short-horizon volatility paths, so coefficient-level identification is harder than one-step variance prediction. In other words, the synthetic phase should mainly be read as a stable initialization step that teaches the network the geometry of heteroskedastic dynamics before real-data adaptation.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{extracted_figures/synthetic_param_scatter.png}

\includegraphics[width=\textwidth]{extracted_figures/synthetic_param_hist.png}
\caption{Synthetic pretraining diagnostics on simulated GARCH(1,1) series. Top: per-series true versus predicted parameters. Bottom: distribution of true and predicted parameters after scaling the process by the training standard deviation.}
\label{fig:synth-diag}
\end{figure}
...
\right]^2,
\]
where $y_m$ is a next-day realized variance proxy derived from squared returns and $\varepsilon$ is a small numerical floor. The training curves in Figure \ref{fig:real-train-curve} show a fast and stable reduction of both training and validation loss on the WIKI panel: in the first real-data run, validation log-MSE drops from about 97 to around 7 in twenty epochs, while the held-out test score is 7.1037. The later zero-aware fine-tuning stage is even stronger, reaching a best validation loss of 4.408 and a test log-MSE of 4.415. This confirms that filtering very small $r^2$ targets and windows with too many zero returns improves calibration on real equities.

\begin{figure}[H]
\centering
\includegraphics[width=0.82\textwidth]{extracted_figures/test_on_real_time_series_cell07_0.png}
\caption{Example real-data training curve for the hybrid NN-GARCH model on WIKI returns. The gap between train and validation remains controlled while both losses decrease sharply over the first fifteen epochs.}
\label{fig:real-train-curve}
\end{figure}
...
\subsection{Real-data volatility forecasting diagnostics}

Before looking at portfolio returns, it is useful to verify that the volatility module itself improves on standard baselines. Figure \ref{fig:real-forecast-panel} compares scratch training, the zero-aware finetuned model, ARCH/GARCH, EWMA, a naive $r_{t-1}^2$ predictor, and a constant-variance baseline on the same filtered real-data evaluation sample. Table \ref{tab:real-forecast-metrics} summarizes the corresponding metrics.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{extracted_figures/test_on_real_time_series_cell24_0.png}
\caption{Predicted variance versus realized $r^2$ on the filtered real-data evaluation sample. The zero-aware finetuned model forms the tightest cloud among the neural methods and substantially improves over scratch training.}
\label{fig:real-forecast-panel}
\end{figure}

\begin{table}[H]
\centering
\caption{Robust real-data forecasting metrics on the common filtered evaluation subset ($n=800$).}
\label{tab:real-forecast-metrics}
\small
\begin{tabular}{lcccc}
\toprule
Method & log-MSE & corr$_{\log}$ & corr$_{\mathrm{level}}$ & $p$-value vs ARCH \\
\midrule
finetune\_zeroaware & 4.8284 & 0.3100 & 0.3280 & $4.86 \times 10^{-24}$ \\
arch\_garch11 & 7.0036 & 0.3215 & 0.2560 & -- \\
ewma\_094 & 7.0061 & 0.3058 & 0.3005 & $9.71 \times 10^{-1}$ \\
scratch & 12.9517 & 0.2586 & 0.2334 & $3.42 \times 10^{-15}$ \\
const & 15.1234 & -- & -- & $9.31 \times 10^{-98}$ \\
naive\_rtm1\_sq & 15.5118 & 0.1615 & 0.2217 & $6.02 \times 10^{-6}$ \\
\bottomrule
\end{tabular}
\end{table}
...
From an applied perspective, the present framework is a strong basis for more realistic investment settings. Long-only constraints, turnover penalties, transaction-cost models, and exposure controls can be added on top of the same forecasting pipeline. In that sense, the project is best viewed as a transparent research contribution for portfolio-risk management, with clear room for extension toward practical and mandate-compatible implementations.
